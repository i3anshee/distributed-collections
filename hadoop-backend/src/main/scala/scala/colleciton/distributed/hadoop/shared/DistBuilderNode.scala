...
class DistBuilderNode(val uri: URI, id: Long)
  extends DistSideEffects(CollectionType, id) with DistBuilderLike[Any, Any] {

  var output: (RuntimePlanNode, Traversable[RuntimePlanNode]) = null

  def result() = throw new RuntimeException("Creation of ditributed collecitons is impossible in cluster nodes!");

  def result(uri: URI) = throw new RuntimeException("Creation of ditributed collecitons is impossible in cluster nodes!");
  ...
  def applyConstraints() {
    throw new UnsupportedOperationException("Constraints can be applied only on client side!!!!")
  }
